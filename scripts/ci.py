#!/usr/bin/env python3
"""CI helpers for spkproof, and a local runner for the workflow itself.
...
Exit codes follow spkproof's own contract:
...
# The README publishes three exit codes to anyone wiring spkproof into a study
# pipeline. Unit tests cover the functions; this covers the promise, through
# the console script, on fixtures shipped in the repository.
#
# `clean_no_findings.csv` exists because of this gate. Before it, the only
# fixture in the repository was the published ECAPA table, which reports
# findings - so exit 0, the code the README documents first, was exercised by
# nothing at all.
_CONTRACT = [
    ("tests/fixtures/clean_no_findings.csv", 0, "a clean study reports nothing"),
    ("tests/fixtures/published_ecapa.csv", 1, "the published contamination signature is a finding"),
    ("tests/fixtures/does-not-exist.csv", 2, "an unreadable file is 'cannot judge', never a finding"),
]
...
def cmd_contract(args: argparse.Namespace) -> int:
    ...
    for relative, expected, description in _CONTRACT:
        ...
        result = subprocess.run(
            [executable, "-m", "spkproof.cli", "check-f0", str(target)],
            cwd=ROOT,
            capture_output=True,
            text=True,
        )
...
def cmd_smoke(args: argparse.Namespace) -> int:
    ...
    probe = (
        "import importlib.metadata as md, spkproof, pathlib, sys;"
        "print(spkproof.__version__);"
        "print(md.version('spkproof'));"
        "print(pathlib.Path(spkproof.__file__).parent)"
    )
    ...
    cli = subprocess.run([executable, "-m", "spkproof.cli", "--version"], capture_output=True, text=True)
